...
dotfiles: ## Installs the dotfiles.
	...
	for file in $(shell find $(CURDIR) -name ".*" -not -name ".gitignore" -not -name ".travis.yml" -not -name ".git" -not -name ".*.swp" -not -name ".gnupg" -not -name ".ssh" ); do \
		f=$$(basename $$file); \
		ln -sfn $$file $(HOME)/$$f; \
	done;
	mkdir -p ${HOME}/.ssh
	ln -sn .ssh/config ${HOME}/.ssh/config
	ln -sn .byoubu ${HOME}/.byoubu
#	gpg --list-keys || true;
